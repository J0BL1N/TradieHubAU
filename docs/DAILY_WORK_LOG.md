...
| --- | --- |
| Area | Frontend, UX Polish |
| Summary | Renamed/mapped original uploaded SFX files in `frontend/public/audio/` to clean, app-safe lowercase hyphenated names. Configured them as selectable choices in the new "App Sounds" tab on the My Profile page. Added toggles and previews for all 8 sound settings (1 message sound, 7 notification sounds) and saved preferences in `localStorage` under `tradiehub_sound_*` keys. Integrated live sound alert playbacks: message tap confirms trigger on incoming chat messages (ignoring active user), and notification chimes play on new alerts. Updated the public audio README.md to list exact filename mappings and usage descriptions. |
