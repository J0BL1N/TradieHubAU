# Profile Trust Live Supabase Deploy Pack

WARNING: Run this SQL in the Supabase SQL Editor on the live Supabase project. Confirm you are in the correct project before running it.

This pack repairs the live profile trust schema/storage gaps when CLI deployment is unavailable.

## Order

1. Run the full `047_repair_profile_trust_live_schema.sql` migration SQL below in Supabase SQL Editor.
2. Run the verification SQL block below.
3. If the app or PostgREST still reports schema-cache errors, refresh the Supabase schema cache/reload the app and try the affected profile page again.

## 1. Full Repair Migration SQL
...
where id in ('profile_media', 'portfolio_images', 'completion_proofs');

select to_regclass('public.tradie_portfolio_items') as tradie_portfolio_items;

select proname
from pg_proc
where proname in (
  'list_my_portfolio_completion_proofs',
  'set_completion_proof_public_portfolio',
  'list_public_tradie_gallery'
)
order by proname;
...
where table_schema = 'public'
  ...
  and column_name in ('business_name', 'headline', 'bio')
order by column_name;
```

## Expected Results

- `profile_media`, `portfolio_images`, and `completion_proofs` should appear in the storage bucket results.
- `tradie_portfolio_items` should be `public.tradie_portfolio_items`, not `NULL`.
- All 3 RPC names should appear: `list_my_portfolio_completion_proofs`, `set_completion_proof_public_portfolio`, and `list_public_tradie_gallery`.
- `business_name`, `headline`, and `bio` should appear for `public_profiles`.
